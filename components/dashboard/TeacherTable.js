...
export default function StudentTable(props) {
    ...
    const handleDeleteTeacher = async (id) => {
        try {
            const route = `/teachers/auth/delete/${id}`
    ...
    const columns = [
        ...
        {
            name: "Reject",
            cell: info => <button onClick={() => handleDeleteTeacher(info._id)} className='py-2 px-3 bg-red-500 rounded-md text-cyan-50 hover:bg-red-600'>
                Reject
            </button>
        }
